(function() {
    ...
    data.headingLevel = options.title_heading_level || 'h3';
    ...
    data.urlFilterBy = $sp.getParameter('filter_by');
    if (data.isMobileApp) {
        data.limit = 5;
    }
    ...
    function markRedKnowledgeBases(contentItems) {
        if (!contentItems || contentItems.length === 0 || !data.redKnowledgeBaseIds || data.redKnowledgeBaseIds.length === 0) {
            return;
        }

        var knowledgeSysIdList = [];
        var knowledgeBaseByArticleId = {};
        var kbKnowledgeGr = new GlideRecord('kb_knowledge');

        for (var i = 0; i < contentItems.length; i++) {
            if (contentItems[i].content_table === 'kb_knowledge' && contentItems[i].content) {
                knowledgeSysIdList.push(contentItems[i].content);
            }
        }

        if (knowledgeSysIdList.length === 0) {
            return;
        }

        kbKnowledgeGr.addQuery('sys_id', 'IN', knowledgeSysIdList.join(','));
        kbKnowledgeGr.query();
        while (kbKnowledgeGr.next()) {
            knowledgeBaseByArticleId[kbKnowledgeGr.getUniqueValue()] = kbKnowledgeGr.getValue('kb_knowledge_base');
        }

        for (var j = 0; j < contentItems.length; j++) {
            if (contentItems[j].content_table !== 'kb_knowledge') {
                contentItems[j].isRedKnowledgeBase = false;
                continue;
            }

            var knowledgeBaseId = knowledgeBaseByArticleId[contentItems[j].content] || '';
            contentItems[j].knowledgeBaseSysId = knowledgeBaseId;
            contentItems[j].isRedKnowledgeBase = data.redKnowledgeBaseIds.indexOf(knowledgeBaseId) > -1;
        }
    }
    ...
    try {
        ...
        if (input && input.action === actions.modifyAISResults) {
			...
			markRedKnowledgeBases(data.modifiedAISResponse);
    ...
    data.filterBy = !gs.nil(data.urlFilterBy) && isValidFilter(data.filterOptions, data.urlFilterBy) ? data.urlFilterBy : data.filterBy;
    try {
        ...
        if (!input || input.action !== actions.showMore) {
            ...
            markRedKnowledgeBases(data.featuredContent);
    ...
    function isValidFilter(filterOptions, filter) {
        if (gs.nil(filterOptions) || filterOptions.length == 0)
            return false;
        for (var i = 0; i < filterOptions.length; i++) {
            if (filterOptions[i].sysId === filter)
                return true;
        }
        return false;
    }
