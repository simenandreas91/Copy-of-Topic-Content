api.controller = function($timeout, $rootScope, $scope, $window, snAnalytics, spAriaUtil, i18n, $sce) {
    var c = this;
    var CARD_VIEW = "card";
    var LIST_VIEW = "list";
    ...
    $scope.widgetTitleHtml = getHeadingHtml();
    $scope.loading = false;
    $scope.contentItems = [];
    ...
    function getHeadingHtml() {
        var headingHtml = '<' + c.data.headingLevel + ' class="title-item">' + c.data.widgetTitle + '</' + c.data.headingLevel + '>';
        return $sce.trustAsHtml(headingHtml);
    }
    ...
    $scope.getExpandSearchURL = function() {
        if (c.data.isNowAssistEnabled) {
            return "?id=nowassistselfservice&spa=1&query=" + encodeURIComponent($scope.searchedTerm) + "&new_chat=true";
        } else {
